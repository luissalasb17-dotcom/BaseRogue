...
def paso_11_motor_defensivo(df, war_bat, awards):
    ...
    df["runs_br_norm"] = ((df["runs_br_peak"] - p02_br) / (p98_br - p02_br)).clip(0, 1)
...
def paso_15_equipo_y_exportar(df, batting, teams, franchises):
    ...
    # INJECTION for 5 random players including castrst01
    test_ids = ['castrst01', 'cabremi01', 'bondsba01', 'troutmi01', 'suzukic01']
    test_df = final[final['playerID'].isin(test_ids)].copy()
    print('--- MATH TRACE FOR 5 PLAYERS ---')
    cols_to_print = ['playerID', 'name', 'ba', 'k_rate', 'contact_raw', 'contact_val', 'iso', 'xbh_rate', 'hr_rate', 'power_raw', 'power_val', 'bb_rate', 'eye_raw', 'eye_val', 'speed_raw_temp', 'speed_raw_adj', 'speed_val', 'defense_val_base', 'gold_gloves', 'defense_val', 'avg_attr_score', 'rarity']
    # Not all cols might be in final, let's see which exist in df before final selection
    
    # We will print from df instead since final drops some cols
    test_df2 = df[df['playerID'].isin(test_ids)]
    cols_avail = [c for c in cols_to_print if c in test_df2.columns]
    for _, row in test_df2.iterrows():
        print(f'\nPlayer: {row['full_name']} ({row['playerID']})')
        for c in cols_avail:
            print(f'  {c}: {row[c]}')
