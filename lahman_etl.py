"""
BaseRogue ETL Pipeline  -  VERSION FINAL 4.0
Lahman + Baseball-Reference war_daily_bat.txt  ->  game_cards.csv
Version : 4.0
Cambios v4.0: Filtrado ESTRICTO pitchers por posicion primaria | PICO 5 temporadas |
              HIBRIDO 60/40 | PA-corrected rates (k_rate, bb_rate) | Sin traits
Uso: pip install pandas numpy && python lahman_etl.py
"""
...
# ===========================================================================
# PASO 5 - METRICAS HIBRIDAS  60% Pico / 40% Carrera
# ===========================================================================
def paso_5_hibrido(career, peak):
    """
    Metrica_Final = 0.60 * Metrica_Pico + 0.40 * Metrica_Carrera
    Si el pico es NaN, se usa la carrera para ambos terminos.
    """
    print(f"\n  PASO 5: Metricas hibridas 60% Pico / 40% Carrera...")
...
def paso_11_motor_defensivo(df, war_bat, awards):
    ...
    if not war_bat.empty:
        war = war_bat.copy()
        # Clean WAR, runs_defense, WAR_def
        for col in ["runs_defense","WAR_def","WAR"]:
        ...
        # Use top 5 seasons by WAR to determine peak defensive metrics
        war_sorted = war.sort_values(["player_ID", "WAR"], ascending=[True, False])
        war_peak = war_sorted.groupby("player_ID").head(5)
        ...
        print(f"  Datos BBRef (Peak 5) para {len(war_career):,} jugadores")
    else:
...
def paso_12_normalizar_por_era(df):
    """
    Normaliza todos los atributos RAW dentro de su Era Tematica
    para eliminar sesgos historicos. Escala de salida: 1-99.
    """
    print("\n  PASO 12: Normalizando por Era (y Defensa Global) (percentil 2-98, escala 1-99)...")
    for raw, out in [
    ...
    # Normalizar defensa globalmente
    df = normalize_globally(df, "defense_base", "defense_val_base")
...
def paso_14_velocidad(df):
    ...
    speed_val (normalizado por Era):
        ...
        40% extra_base_freq = (SB + 3B) / AB
        10% defense_val / 100 (rango posicional)
    """
    print("\n  PASO 14: SPD hibrido 50/40/10 (normalizado por Era)...")
...
def paso_15_equipo_y_exportar(df, batting, teams, franchises):
    ...
    js_lines = [
        ...
        f"// Total: {len(final):,} cartas  |  60% Peak / 40% Career  |  PA-corrected rates",
        "(function() {",
        "  const LAHMAN_POOL = [",
    ]
...
def main():
    ...
    print("  60% Peak / 40% Career  |  PA-corrected rates  |  No traits")
    print("=" * 64)
